...
const ListBusinesses = () => {
  ...
  const getBusinessList = async () => {
    try {
      ...
      setLoading(true)
      const listBusinessResponse = await axios.get(`${process.env.REACT_APP_TBS_TIN_MATCHING_RECIPIENTS_BACKEND_URL}/Business/List`)
            ...
            <>
              <div className='container mx-auto mt-5'>
                <div className="text-center">
                  <p>Before you submit TIN requests in your TaxBandits account, you should have atleast one Business in your account. To add a new business, click 'Create Business' button below. </p>
                  <button className='btn btn_primary shadow btn_md' onClick={navigateToCreateBusiness}>Create Business</button>
                </div>
                <div className='text-start'>
                  <button className='btn_cancel mb-3' onClick={navigateToHomePage}>Back</button>
                </div>
              </div>
            </>
          ) : (
                    ...
                    <>
                      <div className="response-table p-5">
                        <div className='d-flex align-items-center justify-content-between'>
                          <div>
                            <h1 className='head-1'>List Business Details</h1>
                              ...
                              return (
                                <tr key={i}>
                                  ...
                                  <td className='text-center d-flex flex-row justify-content-center'>
                                    <Link className='btn btn-primary status-btn btn_smm me-2' to={`/request/${businessDetail?.BusinessId}/${businessDetail?.BusinessNm}/${businessDetail?.EINorSSN}`}>Request TIN Match</Link>
                                    <Link className='btn btn-primary status-btn btn_smm' to={`/tinmatchlist/${businessDetail?.BusinessId}/${businessDetail?.BusinessNm}/${businessDetail?.EINorSSN}`}>List</Link>
                                  </td>
                                </tr>
                              )
                            })}
                        ...
                        <div className='text-start'>
                          <button className='btn_back mb-3' onClick={navigateToHomePage}>Back</button>
                        </div>
                      </div>
                    </>
                  )}
